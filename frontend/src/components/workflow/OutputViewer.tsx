...
export function OutputViewer({ moduleName, result, onClose }: OutputViewerProps) {
  ...
  const renderVisualAssetOutput = (outputs: Record<string, unknown>) => {
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {outputs.generated_images.map((img: unknown, index: number) => {
              const image = img as { image_url?: string; image_id?: string; prompt_used?: string };
              return (
                    ...
                    <div className="text-sm space-y-1">
                      {image.image_id && (
                        <p className="text-slate-400">
                          <span className="font-medium">ID:</span> {image.image_id}
                        </p>
                      )}
                      {(image as { metadata?: Record<string, unknown> }).metadata && (
                        <div className="text-slate-300">
                          <p className="font-medium">Metadata:</p>
                          <pre className="text-xs bg-slate-900 p-2 rounded mt-1 overflow-x-auto">
                            {JSON.stringify((image as { metadata?: Record<string, unknown> }).metadata, null, 2)}
                          </pre>
                        </div>
                      )}
                    </div>
  ...
  const renderCopyContentOutput = (outputs: Record<string, unknown>) => {
    if (outputs.generated_copies && Array.isArray(outputs.generated_copies)) {
      return (
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            {outputs.generated_copies.map((copy: unknown, index: number) => {
              const copyData = copy as { copy_text?: string; word_count?: number; copy_id?: string; hashtags?: string[]; emojis?: string[] };
              return (
              <Card key={index} className="bg-slate-800 border-slate-700 p-4">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => navigator.clipboard.writeText(copyData.copy_text || '')}
                      className="text-xs"
                    >
                  ...
                  {copyData.copy_text && (
                    <div className="bg-slate-900 p-3 rounded-lg">
                      <p className="text-slate-200 whitespace-pre-wrap">{copyData.copy_text}</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    {copyData.word_count && (
                      <p className="text-slate-400">
                        <span className="font-medium">Word Count:</span> {copyData.word_count}
                      </p>
                    )}
                    {copyData.copy_id && (
                      <p className="text-slate-400">
                        <span className="font-medium">ID:</span> {copyData.copy_id}
                      </p>
                    )}
                  </div>
                  ...
                  {copyData.hashtags && copyData.hashtags.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {copyData.hashtags.map((tag: string, tagIndex: number) => (
                          <span 
                            key={tagIndex}
                  ...
                  {copyData.emojis && copyData.emojis.length > 0 && (
                    <div>
                      <p className="text-slate-400 font-medium mb-2">Suggested Emojis:</p>
                      <div className="flex flex-wrap gap-2">
                        {copyData.emojis.map((emoji: string, emojiIndex: number) => (
                          <span 
                            key={emojiIndex}
  ...
  const renderAudienceAnalysisOutput = (outputs: Record<string, unknown>) => {
    return (
      <div className="space-y-4">
          ...
          <Card className="bg-slate-800 border-slate-700 p-4">
            ...
            <div className="space-y-3">
              {Array.isArray(outputs.audience_segments) ? 
                outputs.audience_segments.map((segment: unknown, index: number) => (
  ...
  const renderContentDistributionSchedulerOutput = (outputs: Record<string, unknown>): React.ReactElement | null => {
    if (outputs.distribution_schedule && Array.isArray(outputs.distribution_schedule)) {
      const distributionSchedule = outputs.distribution_schedule as unknown[];
      return (
        <div className="space-y-4">
          ...
          {(() => {
            const totalAssets = distributionSchedule?.reduce((count: number, item: unknown) => {
              const scheduleItem = item as { content_package?: { asset_urls?: unknown[] } };
              return count + (scheduleItem.content_package?.asset_urls?.length || 0);
            }, 0) || 0;
            
            const postsWithImages = distributionSchedule?.filter((item: unknown) => {
              const scheduleItem = item as { content_package?: { asset_urls?: unknown[] } };
              return scheduleItem.content_package?.asset_urls?.length && scheduleItem.content_package.asset_urls.length > 0;
            }).length || 0;
                    ...
                    {totalAssets > 0 ? (
                      <div className="space-y-1 text-sm">
                        ...
                        <p className="text-blue-400">
                          📊 {postsWithImages}/{distributionSchedule?.length} posts have visual content
                        </p>
          ...
          <div className="h-[600px] w-full">
            <CampaignCalendarWithTimeline timelineData={distributionSchedule as never[]} />
          </div>
          ...
          {outputs.schedule_summary ? (
            <Card className="bg-slate-800 border-slate-700 p-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
                {(() => {
                  const summary = outputs.schedule_summary as { 
                    total_posts?: number; 
                    campaign_coverage?: { timeline_coverage?: string; platform_distribution?: string }; 
                  ...
                  return (
                    <>
                      <div className="space-y-1">
                        ...
                        <p className="text-white font-medium">{summary.total_posts}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-slate-400">Timeline Coverage</p>
                        <p className="text-white font-medium">{summary.campaign_coverage?.timeline_coverage || 'N/A'}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-slate-400">Platform Distribution</p>
                        <p className="text-white font-medium">{summary.campaign_coverage?.platform_distribution || 'N/A'}</p>
                      </div>
                    </>
                  );
                })()}
              ...
              {(() => {
                const summary = outputs.schedule_summary as { posts_by_platform?: Record<string, number> };
                return summary.posts_by_platform && (
                  <div className="mt-4">
          ...
          {(() => {
            const sampleImages = distributionSchedule
              ?.flatMap((item: unknown) => {
                const scheduleItem = item as { content_package?: { asset_urls?: string[] } };
                return scheduleItem.content_package?.asset_urls || [];
              })
              .slice(0, 4) || [];
  ...
  const renderGenericOutput = (outputs: Record<string, unknown>) => {
    return (
      <div className="space-y-4">
